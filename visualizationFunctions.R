...
pitch_plot <-  geom_line(data=pitch_lines, aes(x=x_coordinate, y=y_coordinate, group=pitch_piece))
...
pass_destination_heat_map <- function(pass_event_data) {
  ggplot(pass_event_data, aes( x=pass_end_xlocation, y=pass_end_ylocation)) +
    ...
    geom_line(data=pitch_lines, aes(x=y_coordinate, y=x_coordinate, group=pitch_piece)) +
    geom_point(colour = "red")
    }
  
pass_destination_heat_map(player_pass_events("Paul Labile Pogba"))
